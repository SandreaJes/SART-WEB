...
function respExistenciaSession(resp){
	...
	if(resp==0){
		...
		var cadenaAtributos, streaming = "";
		cadenaAtributos = "preIngreso;"+atrSeg[7];
		streaming = entidad + "|" + cadenaAtributos;	
...
function respTakeCtxPreRevision(resp){
	if(resp==2){
		...
		if( atrPreRevision[12]=="R"){
			document.getElementById('pesoBruto').value= atrPreRevision[14]; 
		}
		...
		SelectObject = document.getElementById("combustible");		
		  ...
		  if( atrPreRevision[12]=="R"){
				document.getElementById('sillas').value= atrPreRevision[27]; 
				document.getElementById('kilometraje').value= atrPreRevision[13];
				document.getElementById('exostos').value= atrPreRevision[15];
				SelectObject = document.getElementById("combustible");		
				  for(index = 0;  index < SelectObject.length;  index++) {			  
				      if(SelectObject[index].value ==atrPreRevision[18]){
				   	     SelectObject.selectedIndex = index;
				   	     if(atrPreRevision[18] == "GAS NATURAL VEHICULAR" || atrPreRevision[9] == "GAS-GASOLINA"){
				  		     $('#gasGasolina').modal('show');		
				  	     }
				      }
				  }
				  var SelectObject = document.getElementById("tipoVehiculo");
				  for(index = 0;  index < SelectObject.length;  index++) {    	   
				      if(SelectObject[index].value ==atrPreRevision[16]){
				   	   SelectObject.selectedIndex = index;
				      }
				  }
				  document.getElementById('nroCertificado').value= atrPreRevision[19];
				  document.getElementById('empresaCertificada').value= atrPreRevision[20];
				  var fV = new Date();
				  fV.setTime(atrPreRevision[21])
				  var mounth = parseInt(fV.getMonth())+parseInt(1);
				  document.getElementById('fechaVencimiento').value= fV.getFullYear()+"-"+mounth+"-"+fV.getDate();
				  if( atrPreRevision[22]=="true"){
					    $("#vidriosPol").prop("checked", true);
				  }
				  if( atrPreRevision[23]=="true"){
					    $("#blindados").prop("checked", true);
				  }
				  if( atrPreRevision[24]=="true"){
					    $("#aplicaEnse").prop("checked", true);
				  }
				  if( atrPreRevision[25]=="true"){
					    $("#aplicAmbulancia").prop("checked", true);
				  }
				  if( atrPreRevision[26]=="true"){
					    $("#aplicBombero").prop("checked", true);
				  }
			}
	}
}
...
function registroPreIngreso(){
		...
		.each(
			function() {
				if(isNaN($(this).val())){
					...
					console.log("this is placeholder is "+sN);
					if (sN != "empresaCertificado") {
	...
	var frm = document.getElementById("blindado");
	...
	var frm = document.getElementById("vidriosPolarizados");
	$("input[type=checkbox]", frm)
	   .each(
		 function() {			 	
		 if($(this).is(':checked')){			 
	...
	var frm = document.getElementById("aplicaBombero");
	$("input[type=checkbox]", frm)
	   .each(
		 function() {			 	
		 if($(this).is(':checked')){
	...
	var frm = document.getElementById("aplicaAmbulancia");
	$("input[type=checkbox]", frm)
	   .each(
		 function() {			 	
		 if($(this).is(':checked')){
	...
	if (envio == true) {
        ...
        var cadenaAtributos, streaming = "";
        streamingDatosAtrr= streamingDatosAtrr+";"+document.getElementById("tipoVehiculo").value +";4";
       
        cadenaAtributos = streamingDatosAtrr+"%"+strLegalizacion+";"+document.getElementById("combustible").value + ";"+document.getElementById("fechaVencimiento").value;
