...
function respFindCtxPreRevision(resp){	
	...
	if( atrSeg[0]=="2"){
		...
		document.getElementById('divPin').style.visibility = "visible";	
		document.getElementById('nroPin').focus();
		document.getElementById('nroPin').value="N/A";
		document.getElementById('nroPin').select();
		document.getElementById("pinSicov").checked= "visible"
	}	
}
...
function showNroPin(obj){
	if (obj.checked) {
		document.getElementById('divPin').style.visibility = "visible";
		document.getElementById('nroPin').focus();
	}else{
		document.getElementById('divPin').style.visibility = "hidden";
	}
}
...
function validacionBeforeTrasmision(){	
	...
	if(document.getElementById("pinSicov").checked==false){
		options.message ="Disculpe, No ha Confirmado si se Compro el Respectivo PIN para esta Revision ";
		options.subtitle=" Validacion Trasmision";
	    eModal.alert(options);	    
	    envio =false;
	    return false;
	...
	if(document.getElementById("nroPin").value.length<=2){
		options.message ="Disculpe, No ha especificado el Nro. de Pin para esta Revision ";
		options.subtitle=" Validacion Trasmision";
		obj="nroPin";
	    eModal.alert(options);	    	    
	    envio =false;
	...
	blob = new Blob([content], { type: "text/plain"});
	formData.append("nroPin", blob,document.getElementById("nroPin").value );	
