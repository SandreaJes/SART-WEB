...
function respExistenciaSession(resp){
	...
	if(resp==0){
		...
		var cadenaAtributos, streaming = "";
		cadenaAtributos = "medidasPSI;"+atrSeg[7];		
		streaming = entidad + "|" + cadenaAtributos;	
		doAjax('./ServicioBasicoControler.do', streaming,'respTakeCtxPreRevision', 'get', 0);
    }
}

function habRepuesto(obj){	
	if($(obj).is(':checked')){
		$("#ejeRepues").prop("disabled", false);
		var indice = Math.floor(Math.random() * rangoAutomovil.length);
		$("#ejeRepues").val(rangoAutomovil[indice+1]);
		
	}else{
		$("#ejeRepues").prop("disabled", true);
		$("#ejeRepues").val(" ");
	}	
}

function respTakeCtxPreRevision(resp){
	console.log("REPONSE SERVIDOR "+resp)	
	
	var atrPreRevision = resp.split(";");	
	console.log("leng arreglo "+atrPreRevision.length)
	...
	if( atrPreRevision[3]=="MOTOCICLETA"){
		...
		if( atrPreRevision[14]=="true"){
		    $("#scoter").prop("checked", true);
		}
		document.getElementById('capaLLantaRepuesto').style.display='none';		
	...
	if( atrPreRevision[3]=="AUTOMOVIL"){
		...
		document.getElementById('pacha').style.display='none';
		document.getElementById("lblCtx").innerHTML="";
		document.getElementById('eje2IzqInt').value="";
	...
	if( atrPreRevision[3]=="BUS" || atrPreRevision[3]=="BUSETA" || atrPreRevision[3]=="MICROBUS"  ){
		...
		if( atrPreRevision[15]=="true"){
		    $("#pacha").prop("checked", true);
		}
	}
	...
	if( atrPreRevision[3]=="TRACTOCAMION" || atrPreRevision[3]=="CAMION" || atrPreRevision[3]=="VOLQUETA" ){
		...
		if (atrPreRevision.length>12) {
     		if (atrPreRevision[12].length > 4) {
			console.log(" ga de extracion de estado " + atrPreRevision[12])
			var SelectObject = document.getElementById("selectNroEjes");
			for (index = 0; index < SelectObject.length; index++) {
				if (SelectObject[index].value == atrPreRevision[13]) {
					SelectObject.selectedIndex = index;
				}
			}

			var SelectObject = document.getElementById("rin");
			for (index = 0; index < SelectObject.length; index++) {
				if (SelectObject[index].value == atrPreRevision[12]) {
					SelectObject.selectedIndex = index;
				}
			}
...
function registroPreRevision(){	
   ...
   var envio =true;	
    ...
    if($("#llantaRespVisible").is(':checked')){
    	exitLLanRep="true";		
	}else{
		exitLLanRep="false";
	}
	...
	if (envio == true) {		
        ...
        var cadenaAtributos, streaming = "";
        cadenaAtributos = nroEjes +";"+document.getElementById("rin").value+";"+";"+exitPacha+";"+exitScoter+";"+exitLLanRep+";"+document.getElementById("ejeRepues").value+";"+streamingPsiEje;
        streaming = entidad + "|" + cadenaAtributos;	
...
function respRegistroPSI(resp){	
	...
	if(claseVehiculo=="MOTOCICLETA"){
		resource="PreReVehicularMoto.html";
	}else{
