...
function respTakeCtxPreRevision(resp){
	...
	if (atrPreRevision[11] == "N") {
		...
		contexto.drawImage(imgPlacaExtra, 0, 0, 80, 40);
		var str =atrPreRevision[0];		
		contexto.font="bold 15px sans-serif";
...
function retCapa(){
	...
	registroNovedadVehicular(ctxRegNovedad);
}
...
function registroInventario(){
	var strInventario="";
	var frm = document.getElementById("frmRadio");
	$("input[type=checkbox]", frm)
	   .each(
		 function() {			 	
		 if($(this).is(':checked')){
			 strInventario= strInventario+"true;"			 		   
	     }else{
	    	 strInventario= strInventario+"false;"	    	 
	     }
		 return false;
	});	
	
	frm = document.getElementById("frmEncendedor");
	$("input[type=checkbox]", frm)
	   .each(
		 function() {			 	
		 if($(this).is(':checked')){
	...
	frm = document.getElementById("frmAntena");
	$("input[type=checkbox]", frm)
	   .each(
		 function() {			 	
		 if($(this).is(':checked')){
	...
	var entidad = "servRegistroInventario";
	var cadenaAtributos, streaming = "";
	cadenaAtributos = strInventario;
	streaming = entidad + "|" + cadenaAtributos +"; "+document.getElementById("otrosElementos").value;	
	doAjax('./ServicioBasicoControler.do', streaming,'respServRegistroInventario', 'get', 0);	
}

function respServRegistroInventario(resp){
	if(resp==0){
		optionSession.message ="Disculpe, su session a expirado por inactividad ..!";
		eModal.alert(optionSession);
		setTimeout(function() {scF()}, 1990);	
	}else{
		options.subtitle="Registro Inventario..!";
		options.message =" Se ha REGISTRADO Inventario de una manera Exitosa";
		eModal.alert(options);
		setTimeout(function() {followSing()}, 1577);		
	}
}
var ctxRegNovedad="";
function ctxNovedad(novedad){
	if(novedad=="Abolladura"){
		var obs = document.getElementById("obsAbolladura");
		
	}
	if(novedad=="Rayon"){
		var obs = document.getElementById("obsRayon");		
	}
	if(novedad=="Golpe"){
		var obs = document.getElementById("obsGolpe");		
	}
	ctxRegNovedad=obs.id;
	obs.value= "se encontro "+novedad+" en la Parte "+ctxPreRevVehicular;
}

function registroNovedadVehicular(ctxObservacion){	
	if(cntFotos>4){
	...
	if(strHandler=="no"){		
	  ...
	  formData.append("obsPerpectiva",blob,"Se encontro la siguiente novedad: "+ strHandler[1]+ " en la parte "+ctxPreRevVehicular+" del Vehiculo. "+document.getElementById(ctxObservacion).value);	  
