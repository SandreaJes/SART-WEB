...
function respServFindDatosTakeSoat(resp){	
  var atrSeg = resp.split(";");  
	...
	if (atrSeg[8] == "N") {
		...
		contexto.drawImage(imgPlacaExtra, 0, 0, 160, 70);
		var str =atrSeg[0];		
		contexto.font="bold 27px sans-serif";
		contexto.fillText(str,21,53);
	}
	contexto.closePath();  
...
function registrarDatosSoat(){
	...
	if(document.getElementById("numeroPoliza").value.length<4){
		options.message ="Disculpe, el valor introducido para el NRO. POLIZA  no es Valido ";
		options.subtitle="Datos NO Valido";
	    eModal.alert(options);
	    obj =document.getElementById("numeroPoliza");
	    envio =false;
