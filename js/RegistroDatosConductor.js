...
function respSerFindPropietario(resp) {
	if (resp == 0) {
	    ...
	    if( SelectObject.selectedIndex ==2  ){
	    	document.getElementById('nombres').value = atrSeg[2]+" " +atrSeg[1];				
	    }else{
...
function regisAutorizacionPreRevision(){
	...
	if(tlf.length>12){
		options.message ="Disculpe, el largo del Telefono es mayor a 12  ";
		options.subtitle="Datos NO Valido";		
	    eModal.alert(options);	    
	    envio =false;
	    return false;
	...
	if(envio==true){
		...
		var cadenaAtributos, streaming = "";		   
		cadenaAtributos = document.getElementById("idPreRevision").value+";"+ document.getElementById("cedula").value+";"+ document.getElementById("nombres").value+"; "+document.getElementById("apellidos").value +" ; "+ document.getElementById("identificador").value+";"+document.getElementById("direccion").value+";"+ document.getElementById("telefono").value+";"+ document.getElementById("correoElectronico").value;
	    streaming = entidad + "|" + cadenaAtributos;			
